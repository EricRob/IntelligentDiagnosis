...
def epoch_size(mode, batch_size, num_steps, image_size):
  ...
  if mode == 'test':
    total_test_sequences = os.path.getsize(os.path.join(FLAGS.data_path, 'test.bin')) // (image_size * image_size * 3 * num_steps + meta_data_bytes)
    epoch_size = int(1.5*((total_test_sequences * 2) //  batch_size))
...
class SeqInput(object):
  ...
  def __init__(self, config, mode, name=None):
    ...
    if mode == 'test': 
      self.input_data, self.targets, self.subjects, self.names, self.coords, self.features = reader_features.read_test_data([os.path.join(FLAGS.data_path, str("test.bin"))], 
        config)
    else: 
