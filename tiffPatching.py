...
def save_patches (originalPath, patches, patchSize, overlap, sampleSize):
    ...
    for i in range(patches.shape[0]):
        for j in range(patches.shape[1]):
            ...
            img = Image.fromarray(patches[i,j,0,:,:,:], 'RGB')
            img = img.resize((sampleSize,sampleSize), Image.ANTIALIAS)
            img.save(name)
...
if __name__ == '__main__':
    ...
    numPatches = save_patches(originalPath, patches, patchSize, overlap, sampleSize)
