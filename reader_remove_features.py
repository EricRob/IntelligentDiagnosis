...
def _generate_half_batch(record_data, min_queue_examples, batch_size, num_steps, test_mode):
	...
	features = tf.reshape(features, [batch_size // 2, record_data.num_features-1])
...
def process_removal_slice(index, byte_length, record_data, remove_idx, block_size=1):
	idx = index
	end_idx = index + byte_length
	start = tf.reshape(tf.strided_slice(record_data, [idx], [idx + remove_idx]), [remove_idx])
	# middle = tf.zeros([1], tf.float64)
	end = tf.reshape(tf.strided_slice(record_data, [idx + remove_idx + 1], [end_idx]), [byte_length - remove_idx - 1])
	result = tf.concat([start, end], 0)
